defmodule AshPki.Plug.MTLS do
  ...
  ## Options
    ...
    * `:certificate` — the `Certificate` resource module to look the
      peer cert up in. Defaults to `AshPki.Certificate`. Pass your own
      module (e.g. `MyApp.Certificate`) when the app owns the resource.
    ...
    * `:require_known_certificate` — if `true` (default), the cert must be
      present in the configured `Certificate` resource. Set to `false`
      for endpoints that should accept any chain-valid client cert
      (e.g. enrollment).
  ...
  def call(conn, opts) do
    cert_module = Keyword.get(opts, :certificate, AshPki.Certificate)
    require_known? = Keyword.get(opts, :require_known_certificate, true)

    trust_anchors_fun =
      Keyword.get(opts, :trust_anchors, fn ->
        AshPki.TrustStore.trust_anchors(
          certificate_authority:
            AshPki.Resource.Certificate.Info.pki_certificate_authority!(cert_module)
        )
      end)
    ...
    with {:ok, peer_cert} <- read_peer_cert(conn, header_mode),
         ...
         {:ok, actor} <- build_actor(cert_module, peer_cert, require_known?) do
  ...
  defp build_actor(cert_module, peer, require_known?) do
